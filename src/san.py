...
def generate_mask(new_data, dropout_p=0.0, is_training = True):
    if not is_training:
        dropout_p = 0.0
...
class SAN(nn.Module):
    ...
    def forward(self, x, h0, x_mask):
        ...
        if self.mem_type == 1:
            mask = generate_mask(self.alpha.data.new(x.size(0), self.num_turn), self.mem_random_drop, is_training = self.training) 
            mask = [m.contiguous() for m in torch.unbind(mask, 1)]
            start_scores_list = [mask[idx].view(x.size(0), 1).expand_as(inp) * F.softmax(inp, dim=1) for idx, inp in enumerate(start_scores_list)] 
            end_scores_list = [mask[idx].view(x.size(0), 1).expand_as(inp) * F.softmax(inp, dim=1) for idx, inp in enumerate(end_scores_list)] 
            # avg score
