...
class ContextualEmbed(nn.Module):
    def __init__(self, path, vocab_size, emb_dim=300, embedding=None, padding_idx=0, mix_layer=False, num_layer=4, dropout=None):
        ...
        if self.mix_layer:
            ...
            for k in range(num_layer):
                scalar_mix = ScalarMix(2, do_layer_norm=False)
    ...
    def forward(self, x_idx, x_mask):
        if self.embedding is None: # already embeded
            x_hiddens = x_idx
        else:
        ...
        if not self.mix_layer: return [output1, output2]

        # compute the elmo representations
        representations = []
        for i in range(len(self._scalar_mixes)):
            scalar_mix = getattr(self, 'scalar_mix_{}'.format(i))
            representation = scalar_mix([output1, output2], x_mask)
            if self._dropout is not None: representation = self._dropout(representation)
